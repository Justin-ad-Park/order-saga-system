...
› 현재 진행중인 프로젝트는 맥북에서 개발하고 있고, 멀티모듈 프로젝트로 구성되어 있어.
로컬에 도커+쿠버네티스를 설치해서 MySQL, MSA(본 프로젝트의 각 모듈), Kafka 클러스터를 쿠버네티스로 구성했어.

우선 MSA는 1. 주문 오케스트레이션 MSA 1-1. orderID 발행
2. 쿠폰MSA의 쿠폰 예약 API, 포인트MSA의 포인트 예약 API를 리액티브로 동시 호출 
3. 2.가 모두 성공을 응답받으면 outbox transaction 상태 업데이트 및 쿠버네티스 토픽에 성공 이벤트 발행 
3. 2. 중에서 하나라도 실패 시 outbox transaction 상태를 실패로 업데이트 및 토픽에 실패 이벤트 발행

여기까지 현재 개발된 내용의 요약이고, 이제 아래 4.부터 개발을 진행하려고 해.

우선 OrderSagaConsumer 라는 프로젝트로 모듈을 추가해줘. 모듈은 Kafka 컨슈머 프로세스를 실행할꺼고,  
